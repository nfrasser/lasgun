...
use ::lasgun::{ output, scene::{Scene, Options, Aggregate} };
...
fn spooky() -> Scene {
    ...
    let skull = scene.add_mesh_at(obj_path("skull").as_path()).unwrap();
    let plane = scene.add_mesh_at(obj_path("plane").as_path()).unwrap();

    // Add materials to the scene
    let floor = scene.add_plastic_material([0.8, 0.7, 0.7], [0.0, 0.0, 0.0], 0.0);
    let bone = scene.add_plastic_material([0.7, 0.7, 0.5], [0.3, 0.3, 0.3], 0.20);
    let purple = scene.add_plastic_material([0.7, 0.6, 1.0], [0.8, 0.8, 0.8], 0.25);
    let glass = scene.add_glass_material([0.7, 0.6, 1.0], [0.8, 0.8, 0.8], 1.333);
